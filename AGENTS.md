...
Project structure based-on feature/domain organization:
```com.example.project
├── user
│   ├── UserController.java
│   ├── UserService.java
...
### Controller Layer
- Use `@RestController` for REST APIs and `@Controller` for MVC.
- Map endpoints using `@RequestMapping` and HTTP method-specific annotations (`@GetMapping`, `@PostMapping`, etc.).
- Use DTOs for request and response bodies, and avoid exposing internal entities directly.
- Validate incoming requests using `@Valid` and handle validation errors gracefully.
- Handle exceptions at the controller level using `@ExceptionHandler` or globally with `@ControllerAdvice` to return appropriate HTTP status codes and error messages.
...
- Handle exceptions at the service layer and throw custom exceptions where appropriate, which can be handled globally by a `@ControllerAdvice` class.
- Manage transactions using `@Transactional` at the service layer, ensuring that transactional boundaries are clear and well-defined.   

### Repository Layer
- Use Spring Data JPA repositories (`JpaRepository`, `CrudRepository`) for data access.
- Define custom query methods using method naming conventions or `@Query` annotations.
- Avoid complex queries in repositories; consider using a separate query service or specification pattern for complex data retrieval.
